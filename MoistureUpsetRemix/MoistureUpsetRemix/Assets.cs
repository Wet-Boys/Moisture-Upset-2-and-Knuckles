...
internal static class Assets
{
    ...
    {
        if (!_prefab)
            _prefab = Object.Instantiate<Material>(Addressables
                .LoadAssetAsync<GameObject>("RoR2/Base/Golem/GolemBody.prefab").WaitForCompletion()
    ...
    public static Material CopyMaterial(Texture texture)
    {
        if (!_prefab)
            _prefab = Object.Instantiate<Material>(Addressables
                .LoadAssetAsync<GameObject>("RoR2/Base/Commando/CommandoBody.prefab").WaitForCompletion()
                .GetComponentInChildren<SkinnedMeshRenderer>().material);
    ...
    public static T Load<T>(string assetName) where T : Object
    {
        ...
        if (asset is Material material)
        {
            if (material.shader.name.StartsWith("MoistToolkit/StubbedShader"))
                material.shader = Addressables
                    .LoadAssetAsync<Shader>($"RoR2/Base/Shaders/{material.shader.name.Substring(27)}.shader")
                    .WaitForCompletion();
        }
