# TODO (every good mod has a todo list)

- most up to date reference https://github.com/ToastedOven/MoistureUpset/tree/Update-For-Real

- organize it better ffs

- take a look at beetle normal/guard to see if I was an idiot or not

- enemy sfx (see about replacing the calls, not the audio itself so we don't have audio overlap with other mods that borrow sounds like the parent sfx on paladin)

- new better wwise project with all audio in one cause why didn't I do that before

- new better unity project that's actually organized

- Replace Alex Jones with anything else. This bit was funny but I don't want to give the impression that we endorse him (Jack black with a pickaxe? CHICKEN JOCKEY *and it spawns a row of chicken jockeys* (this might be peak actually)) (could replace assets in the gold shores to be the nether)

- maybe shelve item replacements for now

- Make as much as we can client sided. Have a system to check if the host has it to enable functionality like synced roblox skins and such.

- re-balance MLG mode scaling

- Flesh out bonzi buddy with new content

- Just check this file https://github.com/ToastedOven/MoistureUpset/blob/Update-For-Real/MoistureUpset/MoistureUpset/Settings.cs since everything was toggleable. Make sure all of the uses for these configs are complete and that should be 99% of the mod
...
# enemy ideas (not final) https://riskofrain2.wiki.gg/wiki/Monsters
...
- GeepGipGup: Minecraft Slime, Terminal Montage Kirbos

- Halcyonite: 

- Larva: 

- Lunar Chimera (Exploder): Monkey Ball but bomb
...
- Grandparent: Various pipes, cardboard, metal, plastic
...
- Solus Heart: Fraudulent Knight

- SPEX? (shopkeeper in solus zone thing idk): 

# other

- Scrapper: toilet (crapper)

- Finish Jotaro captain?

- Replace void zone audio with MKW creative exercise
